...
* `insecure/main.py` has three deliberate findings: `B324` (HIGH), `B104` (MEDIUM) and `B101` (LOW). `B324` being the only HIGH severity finding is what the `severity-level` / `skips` and `config-file` tests rely on, so keep that property when editing the fixture.
